...
void UAPEntranceUI::NativeConstruct()
{
    ...
    auto DataTable = APGI->GetStringData()->FindRow<FStringDataTable>(FName(*Name), Name); if(!DataTable) return;
    
    TEXT_LevelName->SetText(FText::FromString(DataTable->Content));
}