...
void UAPAttackComponent::NormalAttack(FVector Start, bool CloseAttack, float Multiple, bool bStun, float StunTime, bool Custom, float CustomRadius)
{
	...
	if(Line)
	{
		if(AActor* Actor = HitResult.GetActor())
		{
			...
			Character->GetHitPointComponent()->DistinctHitPoint(HitResult.Location, Actor);
			if(bStun) Character->GetHitPointComponent()->SetCrowdControl(Actor, ECharacterState::Stun, StunTime);
	...
	if(Line)
	{
		...
		for(FHitResult & HitResult : HitResults)
		{
			...
			if(Actor->ActorHasTag(TEXT("Enemy")))
			{
				...
				Character->GetHitPointComponent()->SetKnockBackVec(Start); 
				Character->GetHitPointComponent()->SetKnockBackDist(Dist); 
				Character->GetHitPointComponent()->SetCrowdControl(Actor, ECharacterState::KnockBack, KnockBackTime);
