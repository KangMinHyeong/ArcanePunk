...
void UAPPassiveComponent::RecoveryHP(int32 AddNum)
{
	...
	if(Check <= HPRecoveryPercent)
	{
		...
		OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter.Get())->SetHPPercent(OwnerCharacter.Get(), OriginHP);
	}
}
...
void UAPPassiveComponent::UpdateMaxHP()
{
    ...
    {
		auto SilverData = DataTableGI->GetRowByStruct<FSkillAbilityData>( FName(*RowName[It.Key - 1]), RowName[It.Key - 1]);
    ...
    {
		auto GoldData = DataTableGI->GetRowByStruct<FSkillAbilityData>( FName(*RowName[It.Key - 1]), RowName[It.Key - 1]);
        if(!GoldData) return;
		switch (It.Key)
		{
			break;
    ...
    {
		auto PlatinumData = DataTableGI->GetRowByStruct<FSkillAbilityData>(FName(*RowName[It.Key - 1]), RowName[It.Key - 1]);
	...
	OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter.Get())->SetMaxHP(MaxHP);
...
void UAPPassiveComponent::UpdateATK()
{
	...
	ATK = ATK + ATKPlus;
	OwnerCharacter->SetDefaultATK_Origin(ATK);
	OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter.Get())->SetMaxHP(0.0f);
...
void UAPPassiveComponent::UpdateMaxMP()
{
	...
	OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter.Get())->AddMaxMP(MPPlus);
