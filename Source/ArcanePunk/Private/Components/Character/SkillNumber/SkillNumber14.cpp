...
void USkillNumber14::Spawn_Skill14()
{
	...
	if(SkillRange_Target) SkillRange_Target->SetMaxDist(Skill14_LimitDistance+Skill14_Wide/2);
	if(SkillRange_Target) SkillRange_Target->SetWide(Skill14_Wide);
...
void USkillNumber14::Activate_Skill14()
{
	auto OwnerCharacter = Cast<AArcanePunkCharacter>(GetOwner());
	if(!OwnerCharacter) return; 
	...
	if(SkillRange_Target) SwordClutch->SetDistance(SkillRange_Target->GetTargetDistance() - Skill14_Wide/2);
	if(SkillRange_Target) SwordClutch->SetClutchSpeed(SkillRange_Target->GetMaxDist() / SkillRange_Target->GetTargetDistance());
