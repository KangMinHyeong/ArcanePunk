...
void AArcanePunkPlayerController::InitMainPlayer(AActor* PlayerStart)
{
	...
	{
        auto CurrentPlayer = Cast<AArcanePunkCharacter>(MainPlayer);
        
        CurrentPlayer->PlayerIndex = PlayerIndex;
		GetHUD<AAPHUD>()->BindStatusBar(CurrentPlayer, PlayerIndex++);
    }
...
void AArcanePunkPlayerController::SwapMainPlayerByIndex(uint8 Index)
{
    ...
    {
        bEnhanceSwap = true;
        GetHUD<AAPHUD>()->ClearSwapGauge(Index);
...
void AArcanePunkPlayerController::SwapPlayer(const FVector & Loc, const FRotator & Rot)
{
    ...
    {		
        ...
        MyCharacter->SwapMainPlayer(true, bEnhanceSwap, bSkilling);
