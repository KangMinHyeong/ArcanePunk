...
void UAPTimeManipulationSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
...
void UAPTimeManipulationSubsystem::Deinitialize()
{
    // 모든 타임 딜레이션 정리
...
void UAPTimeManipulationSubsystem::Tick(float DeltaTime)
{
...
void UAPTimeManipulationSubsystem::StartTimeDilation(AActor* Owner, const FTimeDilationSettings& Settings, bool bGlobal, bool bAllowMultiple)
{
    //@Owner
    if (!IsValid(Owner))
    {
    ...
    case ETimeDilationMode::Stop:
    {
        ...
        if (Settings.StopDuration > 0.0f)
        {
            ...
            World->GetTimerManager().SetTimer(
                StopModeTimerHandle,
                FTimerDelegate::CreateUObject(this, &UAPTimeManipulationSubsystem::OnStopModeTimerExpired, Owner, Settings.bSmoothTransition, Settings.TransitionDuration),
    ...
    case ETimeDilationMode::HitStop:
    {
        ...
        World->GetTimerManager().SetTimer(
            HitStopTimerHandle,
            FTimerDelegate::CreateUObject(this, &UAPTimeManipulationSubsystem::OnHitStopTimerExpired, Owner),
            HitStopDuration,
            false
        );
...
void UAPTimeManipulationSubsystem::StartTimeDilation(const TArray<AActor*>& Actors, const FTimeDilationSettings& Settings, bool bGlobal)
{
    // 배열이 비어있는지 확인
    if (Actors.Num() == 0)
    {
...
void UAPTimeManipulationSubsystem::StopTimeDilation(AActor* Owner, bool bSmoothTransition, float TransitionDuration)
{
    //@Owner
    if (!IsValid(Owner))
    {
...
void UAPTimeManipulationSubsystem::StartGlobalTimeDilation(const FTimeDilationSettings& Settings)
{
    UWorld* World = GetWorld();
    if (!World)
    {
...
void UAPTimeManipulationSubsystem::StopGlobalTimeDilation(bool bSmoothTransition, float TransitionDuration)
{
    UWorld* World = GetWorld();
    if (!World)
    {
...
void UAPTimeManipulationSubsystem::StartLocalTimeDilation(AActor* Owner, const FTimeDilationSettings& Settings)
{
    if (!IsValid(Owner))
    {
...
void UAPTimeManipulationSubsystem::StopLocalTimeDilation(AActor* Owner, bool bSmoothTransition, float TransitionDuration)
{
    if (!IsValid(Owner))
    {
...
void UAPTimeManipulationSubsystem::ApplyHitStop(AActor* Owner, AActor* Target, const FTimeDilationSettings& Settings, bool bGlobal)
{
    if (!IsValid(Owner))
    {
...
void UAPTimeManipulationSubsystem::UpdateTimeDilation(AActor* Owner, FTimeDilationInfo& DilationInfo, float DeltaTime)
{
    //@World
    UWorld* World = GetWorld();
    if (!World)
...
void UAPTimeManipulationSubsystem::SetTimeDilation(AActor* Owner, const FTimeDilationInfo& DilationInfo, float Value)
{
    //@Wolrd
    UWorld* World = GetWorld();
    if (!World)
...
void UAPTimeManipulationSubsystem::RegisterAnimInstance(AActor* Owner)
{
    //@Owner 유효성 검사
    if (!IsValid(Owner))
    {
    ...
    //@몽타주 블렌딩 아웃 이벤트 바인딩
    AnimInstance->OnMontageBlendingOut.AddDynamic(this, &UAPTimeManipulationSubsystem::OnMontageBlendingOut);

    UE_LOGFMT(LogTimeManipulation, Log, "애니메이션 인스턴스 등록 완료 - 액터: {0}", *Owner->GetName());
}

void UAPTimeManipulationSubsystem::UnregisterAnimInstance(AActor* Owner)
{
    //@Owner 유효성 검사
    if (!IsValid(Owner))
    {
...
void UAPTimeManipulationSubsystem::OnMontageBlendingOut(UAnimMontage* Montage, bool bInterrupted)
{
    //@Montage 유효성 검사
    if (!IsValid(Montage))
    {
...
void UAPTimeManipulationSubsystem::OnHitStopTimerExpired(AActor* Owner)
{
    if (!IsValid(Owner))
    {
...
void UAPTimeManipulationSubsystem::OnStopModeTimerExpired(AActor* Owner, bool bSmoothTransition, float TransitionDuration)
{
    if (!IsValid(Owner))
    {
...
float UAPTimeManipulationSubsystem::CalculateTimeDilationValue(const FTimeDilationSettings& Settings) const
{
    //@CustomDilationValue
...
bool UAPTimeManipulationSubsystem::IsActorTimeDilated(AActor* Owner) const
{
    return ActiveDilations.Contains(Owner);
}

float UAPTimeManipulationSubsystem::CalculateHitStopDuration(const ETimeDilationIntensity& Intensity) const
{
