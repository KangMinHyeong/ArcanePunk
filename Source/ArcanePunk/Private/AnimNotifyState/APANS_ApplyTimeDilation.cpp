#include "AnimNotifyState/APANS_ApplyTimeDilation.h"
...
UAPANS_ApplyTimeDilation::UAPANS_ApplyTimeDilation()
{
    // 기본값 설정
...
void UAPANS_ApplyTimeDilation::NotifyBegin(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float TotalDuration)
{
    //@MeshComp 유효성 검사
    if (!IsValid(MeshComp))
    {
    ...
    //@시간 조작 서브시스템 가져오기
    auto* TimeSystem = GameInstance->GetSubsystem<UAPTimeManipulationSubsystem>();
    if (!TimeSystem)
    {
...
void UAPANS_ApplyTimeDilation::NotifyEnd(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation)
{
    //@MeshComp 유효성 검사
    if (!IsValid(MeshComp))
    {
...
FString UAPANS_ApplyTimeDilation::GetNotifyName_Implementation() const
{
    // 모드와 강도에 따른 표시 이름 생성
    FString ModeName;
