...
void UAPConversationText::CheckFullSkip(float InDeltaTime)
{
    ...
    {
        UE_LOG(LogTemp, Display, TEXT("Your %f"), (SkipPressTime - 0.2f) / 2.79f);
    ...
    if(SkipPressTime >= 3.0f)
    {
        ...
        if(ParentUI.IsValid()) 
        {
            UE_LOG(LogTemp, Display, TEXT("Your ParentUI"));
            ParentUI->StartFullSkip();
        }
        else
        {
            UE_LOG(LogTemp, Display, TEXT("Your not ParentUI"));
        }
    }
}
