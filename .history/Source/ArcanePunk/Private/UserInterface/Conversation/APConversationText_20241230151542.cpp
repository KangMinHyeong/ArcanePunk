...
void UAPConversationText::NativeTick(const FGeometry &MyGeometry, float InDeltaTime)
{
    ...
    if(LetterNum > 0)
    {
        int32 Num = LetterNum - 1;
        float RO = FMath::FInterpTo(TextLetters[Num]->GetRenderOpacity(), 1.0f, InDeltaTime, RenderSpeed);
        TextLetters[Num]->SetRenderOpacity(RO);

        if(TextLetters.Num() == LetterNum && RO >= 0.975f) 
        {
            bTextComplete = true; 
            bTick = false;
...
void UAPConversationText::SetConversation(FSequenceStringDataTable* ContentTextData)
{
    ...
    const float StartTime = GetWorld()->RealTimeSeconds;
    const float TargetTime = StartTime + TextFlowSpeed;

    GetWorld()->GetTimerManager().UnPauseTimer(TextFlowTimerHandle);
    GetWorld()->GetTimerManager().SetTimer(
        TextFlowTimerHandle,
        [StartTime, TargetTime, this]()
        {
            if (GetWorld()->RealTimeSeconds >= TargetTime)
            {
                this->SetTextFlow();
            }
        },
