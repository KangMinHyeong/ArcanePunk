...
{
	return StatusBarWidgets[MainPlayerIndex];
}

bool AAPHUD::CheckSwitchingCoolDown(uint8 Index)
{
    return SwapBar->CheckSwapCoolDown(Index);
}

float AAPHUD::UpdateSwapGauge(float Add)
{
	return SwapBar->UpdateSwapGauge(MainPlayerIndex, Add);
}

float AAPHUD::ClearSwapGauge(uint8 PlayerIndex)
{
	return SwapBar->UpdateSwapGauge(PlayerIndex);
}