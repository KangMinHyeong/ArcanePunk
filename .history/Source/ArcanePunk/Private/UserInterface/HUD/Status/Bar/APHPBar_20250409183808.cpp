...
{
    OwnerCharacter = Character; if(!OwnerCharacter.IsValid()) return;
    const auto PD = OwnerCharacter->GetPlayerStatus();
...
float UAPHPBar::AddSwapGauge(float Add)
{
...
void UAPHPBar::ClearSwapGauge()
{
    TargetSwapGauge = 0.0f;
    CurrentSwapGauge = 0.0f;
    SwapGauge->GetDynamicMaterial()->SetScalarParameterValue(TEXT("Percent"), CurrentSwapGauge / 100.0f);
}

void UAPHPBar::UpdateSwapGauge(float InDeltaTime)
{
    CurrentSwapGauge = FMath::FInterpConstantTo(CurrentSwapGauge, TargetSwapGauge, InDeltaTime, CurrentSwapGaugeSpeed);
    SwapGauge->GetDynamicMaterial()->SetScalarParameterValue(TEXT("Percent"), CurrentSwapGauge / 100.0f);
}
