...
void UChangeCharacterUI::NativeTick(const FGeometry &MyGeometry, float InDeltaTime)
{
    ...
    if(!CoolDown)
    {
        float CoolTime = SkillCoolTime - GetCurrentCoolTime();
        ...
        float CoolTimePercent = GetCurrentCoolTime() / SkillCoolTime;
        CoolTimePercent->GetDynamicMaterial()->SetScalarParameterValue(TEXT("Percent"), CoolTimePercent);
    }
}
