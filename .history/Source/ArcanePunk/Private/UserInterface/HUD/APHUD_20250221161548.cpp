...
void AAPHUD::Init()
{
	PC = Cast<AAPGameModeBase>(GetOwningPlayerController());
	if(!StatusBarUIClass || !GM.IsValid()) return;

	for(int32 i = 0; i<GM->GetPlayerNumbers(); i++)
	{
...
{
	auto StatusBarWidget = StatusBarWidgets[GM->MainPlayerIndex];
...
UAPStatusBar *AAPHUD::GetStatusWidget() const
{
	return StatusBarWidgets[GM->MainPlayerIndex];
}