...
void AAPHUD::Init(TArray<uint8> UI_ID)
{
	...
	GetAPHUD()->GetStatusWidget()->UpdateSkillSlot(OwnerCharacter, CurrentSkillKey);
	GetAPHUD()->GetStatusWidget()->UpdateSkillSlot(OwnerCharacter, CurrentSkillKey);
...
void AAPHUD::UpdateSwapGauge(float Add)
{
	SwapBar->UpdateSwapGauge(MainPlayerIndex, Add);
}