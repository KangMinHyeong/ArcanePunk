...
void AAPSpawnVolume::StartSpawn()
{
	...
	SpawnMonster(FirstStep);	
}

void AAPSpawnVolume::ActivateBattleTimerUI(FSpawnStep* SpawnStep)
{
	...
	if(BattleUI.IsValid()) 
	{
		...
		BattleUI->UpdateBattleUI(TotalStepCount - SpawnStepCount, SpawnStep.TimerValue);
		BattleUI->OnStartingNewWave.AddUObject(this, &AAPSpawnVolume::DespawnMonster);
	}
}

void AAPSpawnVolume::SpawnMonster(FSpawnStep* SpawnStep)
{
	...
	case ESpawnStepType::Timer:
		SpawnStep_Timer = SpawnStep;
		ActivateBattleTimerUI();
		break;
	}

	RespawnMonster(SpawnStep);
}

void AAPSpawnVolume::RespawnMonster(FSpawnStep* SpawnStep) // 리스폰
{
	if(!bActive || !SpawnStep) return;
...
void AAPSpawnVolume::MonsterKilled(AEnemy_CharacterBase *Enemy)
{
	...
	if(Enemies.IsEmpty())
	{
		...
		case ESpawnStepType::Timer:
			RespawnMonster(SpawnStep_Timer);
			break;
		}
	}
}
...
void AAPSpawnVolume::CheckNextSpawn()
{
	...
	{
		SpawnMonster(PreviousFinishSteps[PreviousSpawnDataID]);
	}
}
