...
void AAPBattleSectionBase::OnSpawnLocation()
{
	...
	if(BattleUI.IsValid()) 
	{
		...
		BattleUI->UpdateBattleUI(SpawnData->SpawnStep.Num() - SpawnStep, SpawnMonsterTime);
...
void AAPBattleSectionBase::SpawnMonster()
{
	...
	if(SpawnStep < SpawnData->SpawnStep.Num())
	{
		for( auto SpawnEntry : SpawnData->SpawnStep[SpawnStep].SpawnEntry)
		{
		...
		SpawnStep++;
		if(BattleUI.IsValid()) BattleUI->UpdateBattleUI(SpawnData->SpawnStep.Num() - SpawnStep, SpawnData->SpawnStep[SpawnStep-1]->NextWaveTime);
	}
}
