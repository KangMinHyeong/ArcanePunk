...
void AArcanePunkPlayerController::SetupInputComponent()
{
    ...
    InputComponent->BindAction(TEXT("ChangePlayer_0"), EInputEvent::IE_Pressed, this, &AArcanePunkPlayerController::ChangePlayer_0);
    InputComponent->BindAction(TEXT("ChangePlayer_1"), EInputEvent::IE_Pressed, this, &AArcanePunkPlayerController::ChangePlayer_1);
    InputComponent->BindAction(TEXT("ChangePlayer_2"), EInputEvent::IE_Pressed, this, &AArcanePunkPlayerController::ChangePlayer_2);
}
...
void AArcanePunkPlayerController::ChangeMainPlayer(uint8 Index)
{
    if(MainPlayerIndex == Index) return;
    if(!CheckSwitchingPlayerCondition(Index))
    {
    ...
    PriorCharacter->SwitchingMainPlayer(false, bEnhanceSwap);
    PriorCharacter->RestoreSkills();
    PriorCharacter->DisableInput(this);

    FVector SwitchingLocation = PriorCharacter->GetActorLocation();
    FRotator SwitchingRotation = PriorCharacter->GetActorRotation();
    ...
    SwitchingPlayer(SwitchingLocation, SwitchingRotation);
}

void AArcanePunkPlayerController::SetSwitchingLocation(AArcanePunkCharacter* PriorCharacter, FVector & SwitchingLocation, FRotator & SwitchingRotation)
{
    TArray<AActor*> Enemies;
    UGameplayStatics::GetAllActorsOfClass(GetWorld(), AEnemy_CharacterBase::StaticClass(), Enemies);

    float Dist;
    auto Enemy = UGameplayStatics::FindNearestActor(PriorCharacter->GetActorLocation(), Enemies, Dist);

    float AttackRange = PriorCharacter->GetAttackComponent()->GetBaseAttackDist();
    if(Dist <= AttackRange)
    {
        SwitchingLocation = Enemy->GetActorLocation() - Enemy->GetActorForwardVector() * AttackRange;
        SwitchingRotation = Enemy->GetActorRotation();
    }
    else
    {
        SwitchingLocation = PriorCharacter->GetActorLocation() + FVector(AttackRange, 0.0f, 0.0f);
        SwitchingRotation = PriorCharacter->GetActorRotation(); 
    }

    // 단차?
    SwitchingLocation.Z = PriorCharacter->GetActorLocation().Z;
}

bool AArcanePunkPlayerController::CheckSwitchingPlayerCondition(uint8 Index)
{
    if(!MyCharacter->CheckSwitchingPlayerCondition()) return false;
    if(!GetHUD<AAPHUD>()->CheckSwitchingCoolDown(Index)) return false;
...
void AArcanePunkPlayerController::SwitchingPlayer(FVector Loc, FRotator Rot)
{
    ...
    {		
        ...
        MyCharacter->PressedDash();

        bSwap = true;
        
        SetViewTargetWithBlend(MyCharacter.Get(), SwapDelayTime);
        GetWorldTimerManager().SetTimer(SwapTimerHandle, this, &AArcanePunkPlayerController::SwapPlayer, SwapDelayTime, false);    
    }
}

void AArcanePunkPlayerController::SwapPlayer()
{
    ...
    MyCharacter->SwitchingMainPlayer(true, bEnhanceSwap);
}
