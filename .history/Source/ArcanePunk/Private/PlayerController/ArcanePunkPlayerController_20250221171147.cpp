...
{
    auto HUD = GetHUD<AAPHUD>(); if(!HUD) return;
	...
	{
		HUD->BindStatusBar(Cast<AArcanePunkCharacter>(MainPlayer), PlayerIndex++);
	}
...
void AArcanePunkPlayerController::SwitchingPlayer()
{
    ...
    {		
		...
		MyCharacter->SetActorRotation(PlayerStart->GetActorRotation());
        ...
        HUD->ActivateStatusBar(MainPlayerIndex);
    }
}
...
void AArcanePunkPlayerController::ChangeMainPlayer()
{
    ...
    UnPossess();
    MyCharacter->SetActorLocation(FVector(10000.0f,10000.0f,10000.0f));

    MyCharacter->SetActorHiddenInGame(true);  // 화면에서 숨김
    MyCharacter->SetActorEnableCollision(false);  // 충돌 비활성화
    MyCharacter->SetActorTickEnabled(false);  // 틱 비활성화

    HUD->DeactivateStatusBar(MainPlayerIndex);
    ...
    MyCharacter->SetActorLocation(CurrentLocation);
	MyCharacter->SetActorRotation(CurrentRotation);
