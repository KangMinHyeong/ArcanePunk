...
void AAPGameModeBase::SpawnPlayers()
{
    ...
    {
		Players[MainPlayerIndex]->SetActorHiddenInGame(false);  // 화면에서 숨김
        Players[MainPlayerIndex]->SetActorEnableCollision(true);  // 충돌 비활성화
        Players[MainPlayerIndex]->SetActorTickEnabled(true);  // 틱 비활성화
		FindPlayerStart
