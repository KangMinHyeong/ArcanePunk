...
void ASkillActor::UseSkill()
{
	...
	{
		SetActorLocation(OwnerController->GetSkillTargetRange()->GetActorLocation());
		auto InstallActor = GetWorld()->SpawnActor<AActor>(InstallActorClass, GetActorLocation() + FVector(0.0f, 0.0f, 100.0f), OwnerCharacter->GetActorRotation());
		UE_LOG(LogTemp, Display, TEXT("Your GetSkillTargetRange %f, %f"), OwnerController->GetSkillTargetRange()->GetActorLocation().X, OwnerController->GetSkillTargetRange()->GetActorLocation().Y);
		UE_LOG(LogTemp, Display, TEXT("Your SkillType %f, %f"), GetActorLocation().X, GetActorLocation().Y);
		UE_LOG(LogTemp, Display, TEXT("Your CollisionShape %f, %f"), CollisionShape->GetComponentLocation().X, CollisionShape->GetComponentLocation().Y);
