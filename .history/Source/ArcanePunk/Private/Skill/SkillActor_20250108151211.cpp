...
void ASkillActor::UseSkill()
{
	...
	else if(SkillType == ESkillType::Immediate)
	{
		...
		for (AActor* Actor : Actors)
		{
			...
			float DamageApplied = OwnerCharacter->GetAttackComponent()->ApplyDamageToActor(Actor, OwnerCharacter->GetCurrentATK() * DamageCoefficient, HitResult, true, CrowdControl, CC_Duration);
		}
		bSkilling = false;
...
void ASkillActor::SetActive(bool Active)
{
	...
	else {SkillEffectComponent->Deactivate();}	
...
void ASkillActor::OnOverlap_Projectile(UPrimitiveComponent *OverlappedComp, AActor *OtherActor, UPrimitiveComponent *OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult &SweepResult)
{
	...
	float DamageApplied = OwnerCharacter->GetAttackComponent()->ApplyDamageToActor(OtherActor, OwnerCharacter->GetCurrentATK() * DamageCoefficient, SweepResult, true, CrowdControl, CC_Duration);
