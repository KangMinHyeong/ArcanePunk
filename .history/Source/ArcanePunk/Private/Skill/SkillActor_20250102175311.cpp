...
void ASkillActor::DragSkill()
{
	if(!bDrag) return;

	CutterSpeed = FMath::FInterpTo(CutterSpeed, 0.0f, DeltaTime, Range / ProjectileMovementComponent->InitialSpeed);
	ProjectileMovementComponent->SetVelocityInLocalSpace(FVector(CutterSpeed, 0.0f, 0.0f));
