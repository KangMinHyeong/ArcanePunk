...
void ASkillController::InitializeSkills(FName skillName, AArcanePunkCharacter* OwnerCharacter)
{
	...
	// 스킬 범위 액터 스폰
	SkillRange_Target = GetWorld()->SpawnActor<AAPSkillRange>(SkillRangeClass, OwnerPlayer->GetMesh()->GetComponentLocation(), FRotator::ZeroRotator);
	...
	if(!SkillRange_Target.IsValid()) {
		UE_LOG(LogTemp, Display, TEXT("Your IsValid not"));
		return;
	}
	// 스킬 범위 액터 초기화 및 비활성화
