...
void ASkillActor::UseSkill()
{
	...
	else if(SkillType == ESkillType::Charging)
	{
		...
		SetActorLocation(OwnerCharacter->GetActorLocation() + OwnerCharacter->GetActorForwardVector() * Range * ChargingCoefficient);
    	...
    	float Len = (Range * ChargingCoefficient) / 750.0f;
    	...
    	SkillEffectComponent->SetVariableVec3(TEXT("AddVector"),  FVector(((Range * ChargingCoefficient) - 750.0f) * 0.5f, 0.0f, 0.0f));
...
void ASkillActor::InitSkill(FName SkillNameKey, AArcanePunkCharacter* OwnerCharacterPtr, ASkillController* OwnerControllerPtr)
{
	...
	if (CollisionShape)
	{
		...
		{
			CapsuleComponent->SetCapsuleSize(collisionShapeSize.X, Range * 0.5f);  // Size.X = 반지름, Size.Y = 높이
...
void ASkillActor::ChargindCheck(float DeltaTime)
{
	...
	if(Check)
	{
		...
		OwnerController->GetSkillTargetRange->SetLength(ChargingCoefficient * Range);
	}
	else
	{
