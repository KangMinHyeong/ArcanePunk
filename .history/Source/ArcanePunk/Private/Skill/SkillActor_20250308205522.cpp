...
void ASkillActor::InitSkill(ESkillKey SkillKey, FName SkillNameKey, AArcanePunkCharacter* OwnerCharacterPtr, ASkillController* OwnerControllerPtr)
{
	...
	if (CollisionShape)
	{
		...
		// 충돌처리 및 오버랩함수 바인딩
		CollisionShape->SetCollisionObjectType(ECC_GameTraceChannel3); 
	...
	{
		UE_LOG(LogTemp, Display, TEXT("Your OwnerCharacter->GetAPHUD()"));
		OwnerCharacter->GetAPHUD()->GetStatusWidget()->UpdateSkillSlotImage(CurrentSkillKey, skillDataRow.SkillImage);
	}
}
