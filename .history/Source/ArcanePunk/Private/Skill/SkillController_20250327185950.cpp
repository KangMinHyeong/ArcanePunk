...
void ASkillController::UseSkill(ESkillKey SkillKey, AArcanePunkCharacter* OwnerCharacter)
{
	...
	// 스마트키 체크
	if(SettingGI->GetSmartKey()[(uint8)SkillKey] && SkillDataRow.SkillType != ESkillType::Charging) 
	{
		ActivateSkill();
		return;
	}
...
{
	bool Check = true;
	if(!OwnerPlayer.IsValid()) {UE_LOG(LogTemp, Display, TEXT("Your a")); Check = false;}
	if(OwnerPlayer->GetDoing()) {UE_LOG(LogTemp, Display, TEXT("Your b")); Check = false;}
	if(OwnerPlayer->GetPlayerStatus().StatusData.MP < SkillActor->GetMPConsumption()) {UE_LOG(LogTemp, Display, TEXT("Your c")); Check = false;}
	if(SkillActor->IsSkilling()) {UE_LOG(LogTemp, Display, TEXT("Your d")); Check = false;}
	if(!CheckSkillCool(SkillKey)) {UE_LOG(LogTemp, Display, TEXT("Your e")); Check = false;}
