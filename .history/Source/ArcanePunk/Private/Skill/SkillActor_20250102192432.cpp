...
void ASkillActor::UseSkill()
{
	...
	if(!bAttachedEffect)
	{
		...
		SkillEffectComponent->SetWorldRotation(GetActorRotation()); 
		SkillEffectComponent->SetUsingAbsoluteRotation(true);
	}
	SkillEffectComponent->Activate();	

	UE_LOG(LogTemp, Display, TEXT("Your GetWorldRotation %f, %f"), SkillEffectComponent->GetComponentRotation().Yaw, SkillEffectComponent->GetComponentRotation().Roll, SkillEffectComponent->GetComponentRotation().Pitch);
