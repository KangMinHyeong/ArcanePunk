...
void ASkillActor::UseSkill()
{
	...
	GetWorld()->GetTimerManager().SetTimer(DestroyTimerHandle, TimerDelegate, Range / ProjectileMovementComponent->InitialSpeed, false);
...
void ASkillActor::EnhanceSkill(int32 EnhanceNumber)
{
	...
	if(EnhanceNestingData.Contains(EnhanceNestingData))
	{
		CurrentNestingNumber = EnhanceNestingData[EnhanceNestingData];
	}
...
void ASkillActor::OnOverlap(UPrimitiveComponent *OverlappedComp, AActor *OtherActor, UPrimitiveComponent *OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult &SweepResult)
{
	...
	float DamageApplied = OwnerCharacter->GetAttackComponent()->ApplyDamageToActor(OtherActor, OwnerCharacter->GetCurrentATK() + PlusDamage, SweepResult, true);
