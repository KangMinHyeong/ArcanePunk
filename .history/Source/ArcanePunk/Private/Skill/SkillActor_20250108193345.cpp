...
void ASkillActor::ChargindCheck(float DeltaTime)
{
	...
	if(Check)
	{
    	...
    	ChargingCoefficient = FMath::FInterpConstantTo(ChargingCoefficient, MaxCharging, DeltaTime, 5.0f);
	}
	else
	{
...
void ASkillActor::ChargingSkill()
{
	...
	ChargingEffect = UNiagaraFunctionLibrary::SpawnSystemAtLocation(GetWorld(), ChargeEffect, OwnerCharacter->GetActorLocation(), OwnerCharacter->GetActorRotation());
}
