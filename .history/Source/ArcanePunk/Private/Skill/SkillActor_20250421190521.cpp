...
void ASkillActor::UseSkill()
{
	...
	OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter)->UpdateMPBar(MPConsumption, true);
	OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter)->OperateSkillSlot(CurrentSkillKey);
...
void ASkillActor::ChargindCheck(float DeltaTime)
{
	...
	if(Check)
	{
		OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter)->ChargeGauge(ChargingCoefficient - 1.0f , MaxCharging - 1.0f);
...
{
	OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter)->StartCoolTimeSlot(CurrentSkillKey, CoolTime);
}

void ASkillActor::ChargingSkill()
{
	OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter)->ChargeEnd(true);
	bCharging = true;
...
void ASkillActor::ChargingEnd()
{
	...
	bCharging = false;
	OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter)->ChargeEnd(false);
...
void ASkillActor::UpdateSkillSlotImage()
{
	...
	{
		OwnerCharacter->GetAPHUD()->GetStatusWidget(OwnerCharacter)->UpdateSkillSlotImage(CurrentSkillKey, SkillSlotImage);
	}
}