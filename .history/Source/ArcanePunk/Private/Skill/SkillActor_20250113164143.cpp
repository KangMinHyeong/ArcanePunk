...
void ASkillActor::UseSkill()
{
	...
	else if(SkillType == ESkillType::Installation)
	{
		...
		auto InstallActor = GetWorld()->SpawnActor<AActor>(InstallActorClass, GetActorLocation(), OwnerCharacter->GetActorRotation(), SpawnParams);
		if(InstallActor) InstallActor->SetOwner(OwnerCharacter);
	}
