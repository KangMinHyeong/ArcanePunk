...
void ASkillActor::UseSkill()
{
	...
	else if(SkillType == ESkillType::Charging)
	{
		...
		SetActorLocation(OwnerCharacter->GetActorLocation() + OwnerCharacter->GetActorForwardVector() * HalfHeight * 0.55f);
...
void ASkillActor::InitSkill(FName SkillNameKey, AArcanePunkCharacter* OwnerCharacterPtr, ASkillController* OwnerControllerPtr)
{
	...
	if (SkillEffectComponent)
	{
		...
		float Wid = skillDataRow.CollisionSize.X / 50.0f;
    	float Len = Range / 750.0f;
    	...
    	SkillEffectComponent->SetVariableVec3(TEXT("AddVector"),  FVector((Range - 750.0f) * 0.5f, 0.0f, 0.0f));
	}
