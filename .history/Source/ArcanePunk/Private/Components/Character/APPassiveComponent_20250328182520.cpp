...
void UAPPassiveComponent::RecoveryHP(int32 AddNum)
{
	...
	if(Check <= HPRecoveryPercent)
	{
		...
		OwnerCharacter->GetAPHUD()->GetStatusWidget()->SetHPPercent(OwnerCharacter.Get(), OriginHP);
	}
}
...
void UAPPassiveComponent::UpdateMaxHP()
{
	...
	OwnerCharacter->GetAPHUD()->GetStatusWidget()->SetMaxHP(MaxHP);
...
void UAPPassiveComponent::UpdateATK()
{
	...
	ATK = ATK + ATKPlus;
	OwnerCharacter->SetDefaultATK_Origin(ATK);
	OwnerCharacter->GetAPHUD()->GetStatusWidget()->SetMaxHP(0.0f);
...
void UAPPassiveComponent::UpdateMaxMP()
{
	...
	OwnerCharacter->GetAPHUD()->GetStatusWidget()->AddMaxMP(MPPlus);
