...
void UAPMovementComponent::TickLookAt(float DeltaTime)
{
	...
	FRotator LookAtRot = UKismetMathLibrary::FindLookAtRotation(OwnerCharacter->GetActorRotation(), TargetLookAt);
	FRotator CurrentRot = FMath::RInterpConstantTo(OwnerCharacter->GetActorRotation(), LookAtRot, DeltaTime, RotSpeed);
	GetOwner()->SetActorRotation(Current);
	
	if(abs(LookAtRot.Yaw - CurrentRot.Yaw) < 0.01f) 
	{
		bLookAt = false;
	}
}
...
{
	DashLocation = GetOwner()->GetActorLocation() + GetOwner()->GetActorRightVector() * DashLength;
