...
void UAPCharacterAuraComponent::BeginPlay()
{
	...
	OwnerPC = Cast<AArcanePunkPlayerController>(OwnerCharacter->GetController()); 
	
	if(!OwnerPC.IsValid()) {UE_LOG(LogTemp, Display, TEXT("Your not OwnerPC")); return;}
	else{UE_LOG(LogTemp, Display, TEXT("Your OwnerPC"));}
